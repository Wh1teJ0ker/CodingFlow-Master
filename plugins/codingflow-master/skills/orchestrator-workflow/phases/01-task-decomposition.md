...
每个版本号是一个独立的迭代周期。任务编号 `T<n>` 在**版本作用域内**从 `T1` 开始递增，**不跨版本累加**。

- 版本 `v1.2.0` 的任务为 `T1`、`T2`、`T3`...
- 版本 `v1.3.0` 的任务**重新从 `T1` 开始**，与 `v1.2.0` 的 `T1` 互不冲突
- 新版本 = 新的迭代周期 = 编号重置
- 同版本内任务编号唯一且递增，不复用已删除任务的编号
...
任务拆分完成后，默认要求对应实现也保持清晰的 commit 边界：
...
goal: |                  # 整体目标（用户视角的一句话）
  ...
tasks:
  - id: T1              # 版本内从 T1 递增，不跨版本累加（详见"任务编号规则"）
    title: ...
