...
本规范定义 CI / Release workflow 模板必须满足的通用要求。发布流程要求见 [`RELEASE-STANDARD.md`](./RELEASE-STANDARD.md)。可复制模板位于 `../../assets/templates/`。
...
- 包管理器安装必须使用锁文件确定性命令（如 `npm ci`、`pnpm install --frozen-lockfile` 等价命令）
- 缓存键绑定对应 lockfile，避免跨包管理器误命中
