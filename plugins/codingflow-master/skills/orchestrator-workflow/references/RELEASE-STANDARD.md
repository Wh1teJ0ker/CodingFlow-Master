...
本规范定义版本发布流程必须满足的通用要求。GitHub Actions 层面的执行约束见 [`GITHUB-ACTIONS-STANDARD.md`](./GITHUB-ACTIONS-STANDARD.md)；Release QA 审计流程见编排过程的 [`../phases/07-release-qa-audit.md`](../phases/07-release-qa-audit.md)。版本文档模板位于 `../../assets/templates/`。
...
每个版本必须有一个权威版本源（如项目清单文件中的 `version` 字段），其余位置同步自它：
