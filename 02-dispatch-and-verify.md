...
| 任务状态总表 | `handoff/TASK-BOARD.md` | 主会话 | 所有人 | Release QA `qa_passed` 且版本状态同步后 |
...
选 `ready`（`depends_on` 全部 `verified_complete`）的任务，派 coder 子 Agent，把对应任务文件作为唯一输入源。子 Agent 遵守 [`04-coder-spec.md`](./04-coder-spec.md) 定义的 Coder 规范。
...
- coder 的实施边界、验证要求和 REPORT 格式由 [`04-coder-spec.md`](./04-coder-spec.md) 定义
...
派 reviewer 子 Agent，把 HANDOFF 与 coder REPORT 作为唯一输入源。子 Agent 遵守 [`05-reviewer-spec.md`](./05-reviewer-spec.md) 定义的 Reviewer 规范。
...
- reviewer 的审查边界、缺陷格式和 verdict 取值由 [`05-reviewer-spec.md`](./05-reviewer-spec.md) 定义
...
2. **同步进度**：按 [`06-progress-sync.md`](./06-progress-sync.md) 更新 `docs/versions/<ver>/更新日志.md` 中对应任务的状态，确保文档反映真实进度。

> ⚠️ 这两步缺一不可。只清理 handoff 不同步 docs，会导致文档与实际脱节；只同步 docs 不清理 handoff，会导致工作区堆积过期交接文件。

## Phase 6 — 推进到所有任务 verified_complete

重复 Phase 2–5，直到 `TASK-BOARD.md` 里所有任务 `status: verified_complete`。随后进入 [`03-e2e-acceptance.md`](./03-e2e-acceptance.md)；E2E 通过后还必须进入 [`07-release-qa-audit.md`](./07-release-qa-audit.md)，不能直接发布版本。
